...
int main(int argc, char *argv[]) {
	...
	while (1) {
        ...
        } else if (event.xany.window==childWindow.handle) {
            eventWindow = &childWindow;
        } else {
        ...
        switch (event.type) {
            ...
            case KeyPress:
                ...
                if (XK_h == keysym) {
                    ...
                    sendClientEvent(cxt, eventWindow->handle, cxt->_NET_WM_STATE, 4, 1, 
                            XInternAtom(cxt->display, "_NET_WM_STATE_MAXIMIZED_VERT", False),
                            XInternAtom(cxt->display, "_NET_WM_STATE_MAXIMIZED_HORZ", False), 0);
                } else if (XK_s == keysym) {
    ...
    XDestroyWindow(cxt->display, childWindow.handle);
