...
//    self->frame = av_frame_alloc();
    ...
    self->sws = sws_getContext(
            in_width, in_height, AV_PIX_FMT_RGB565,
...
void encoder_encode(encoder_t *self, void *rgb_pixels, void *encoded_data, size_t *encoded_size) {
    ...
    int in_linesize[1] = {self->in_width * 2};
    sws_scale(self->sws, (const uint8_t * const*)in_data, in_linesize, 0, self->in_height, self->frame->data, self->frame->linesize);
