/**
 * @brief XShapeCombineRectangles可以截取窗口，使用XShapeCombineRectangles时要链接-lXext
 * 
 * 编译：gcc xlib_draw.c xlib_jpeg.c  -lX11 -lXext -ljpeg -lm
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
	...
	while (1) {
        ...
        switch (event.type) {
            ...
            case KeyPress:
                ...
                if (XK_s == keysym) {
                    updatePixmap(cxt, eventWindow, "test2.jpg", eventWindow->x, eventWindow->y);
                    XCopyArea(cxt->display, cxt->primary, eventWindow->handle, eventWindow->gc,
		                    eventWindow->x, eventWindow->y, eventWindow->width, eventWindow->height, 0, 0);
                } else if (XK_c == keysym) {
                    //XShapeCombineRectangles可以截取窗口，下面的代码实现的效果就是给窗口挖了个洞
                    XRectangle* xrects;
                    int nrects = 4;
                    int i = 0;
		            ...
		            xrects[3].width = 500;
		            xrects[3].height = 100;
                    XShapeCombineRectangles(cxt->display, eventWindow->handle, ShapeBounding, 0, 0, xrects, nrects, ShapeSet, 0);
                    free(xrects);
                } else if (XK_d == keysym) {
                    XSetFillStyle(cxt->display, eventWindow->gc, FillSolid);
	                XFillRectangle(cxt->display, cxt->primary/*eventWindow->handle*/, eventWindow->gc,
			                eventWindow->x, eventWindow->y,
			                50, 50);
                    XCopyArea(cxt->display, cxt->primary, eventWindow->handle, eventWindow->gc,
		                    eventWindow->x, eventWindow->y, eventWindow->width, eventWindow->height, 0, 0);
                } else if (XK_q == keysym) {
